...
/**
 * Тест для проверки /tree endpoint БЕЗ параметров
 */
...
const logger = new Logger('TreeNoParamsTest');

async function testTreeWithoutParams() {
  try {
    logger.info('=== Тестирование /tree endpoint БЕЗ параметров ===\n');
    ...
    // Пробуем получить дерево для структуры 138 БЕЗ параметров
    const testStructureId = '138';
    logger.info(`3. Получение дерева структуры ${testStructureId} через /tree endpoint БЕЗ параметров...`);
    
    try {
      // Вызываем напрямую метод API клиента с пустыми параметрами
...
testTreeWithoutParams().catch(error => {
